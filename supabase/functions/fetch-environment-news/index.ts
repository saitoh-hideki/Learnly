...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (sourcesError || !sources || sources.length === 0) {
      return new Response(
        JSON.stringify({ error: 'No RSS sources found for environment category' }),
        { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    console.log(`Processing ${sources.length} RSS sources for environment category`)
    ...
    console.log(`Environment RSS fetch completed. Articles: ${allArticles.length}, Errors: ${errors.length}`)
    ...
    console.error('Environment RSS fetch function error:', error)
    return new Response(
      JSON.stringify({ 
        error: 'Failed to fetch environment RSS news',
