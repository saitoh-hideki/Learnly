...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (sourcesError || !sources || sources.length === 0) {
      return new Response(
        JSON.stringify({ error: 'No RSS sources found for technology category' }),
        { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    console.log(`Processing ${sources.length} RSS sources for technology category`)
    ...
    console.log(`Technology RSS fetch completed. Articles: ${allArticles.length}, Errors: ${errors.length}`)
    ...
    console.error('Technology RSS fetch function error:', error)
    return new Response(
      JSON.stringify({ 
        error: 'Failed to fetch technology RSS news',
