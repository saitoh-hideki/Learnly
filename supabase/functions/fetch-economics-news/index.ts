...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (sourcesError || !sources || sources.length === 0) {
      return new Response(
        JSON.stringify({ error: 'No RSS sources found for economics category' }),
        { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    console.log(`Processing ${sources.length} RSS sources for economics category`)
    ...
    console.log(`Economics RSS fetch completed. Articles: ${allArticles.length}, Errors: ${errors.length}`)
    ...
    console.error('Economics RSS fetch function error:', error)
    return new Response(
      JSON.stringify({ 
        error: 'Failed to fetch economics RSS news',
